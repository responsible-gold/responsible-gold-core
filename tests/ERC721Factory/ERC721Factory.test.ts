...
import { ERC20Factory, ERC721Factory } from "../../typechain-types";
...
describe("ERC721Factory", () => {
  ...
  async function deployFixture() {
    ...
    const ERC721FactoryBase = await ethers.getContractFactory("ERC721Factory");
    const erc721Factory = await ERC721FactoryBase.deploy();
    newLogicAddress = await erc721Factory.getAddress();
    factory = deployer.erc721Factory as ERC721Factory;
  }
  ...
  describe("changeFactory", async () => {
    it("should allow owner to change the rg factory address", async () => {
      const tx = await factory
        .connect(owner)
        .changeFactory(testFactory.address);
      const actualFactory = await factory.rgFactory();
    ...
    it("should fail to change the rg factory address as non-owner", async () => {
